...
from pysstv_modified.pysstv.sstv import byte_to_freq, FREQ_BLACK, FREQ_WHITE, FREQ_VIS_START
from pysstv_modified.pysstv.grayscale import GrayscaleSSTV
...
class testmode(MartinM1):
    COLOR_SEQ = (Color.green, Color.blue, Color.red)
    VIS_CODE = 0x29
    WIDTH = 320
    HEIGHT = 256
    SYNC = 4.862 #was 2.431
    SCAN = 73.216 #was 73.216
    INTER_CH_GAP = 0.143 #was 0.143
    def encode_line(self, line):
        msec_pixel = self.SCAN / self.WIDTH
        image = self.pixels
        for color in self.COLOR_SEQ:
            ...
            for col in range(self.WIDTH):
                pixel = image[col, line]
                yield byte_to_freq(pixel[color.value]), msec_pixel
            yield from self.after_channel(color)
